...
export function LumpsumYearlyCalculator() {
  ...
  const handleSubmitContact = async (e: React.FormEvent) => {
    ...
    await api.submitLead({
      ...
      name, email, phone,
      monthly_investment: Math.round(yearlyInvestment / 12),
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 text-center">
        <h2 className="font-sans text-2xl font-bold text-foreground sm:text-3xl">Yearly Investment Calculator</h2>
        <p className="mt-2 text-muted-foreground">Calculate growth of annual lump sum investments</p>
      </div>
          ...
          {stage === "results" && (
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
              <div className="text-center">
                ...
                <h3 className="font-sans text-2xl font-bold text-card-foreground">Your yearly investments could grow to</h3>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <ul className="mt-2 space-y-1 text-sm text-primary/80">
                      <li>Yearly Investment: {formatCurrency(yearlyInvestment)}</li>
