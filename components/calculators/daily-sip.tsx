...
import { TrendingUp, BarChart3, Calculator, TrendingDown, AlertTriangle, Zap, ArrowRight, CheckCircle2 } from "lucide-react"
import { Button } from "@/components/ui/button"
import { api } from "@/lib/api"
import { computeTrueWealth, type TrueWealth } from "./shared"
import { SliderInput } from "./slider-input"

export function DailySipCalculator() {
  ...
  const totalDays = tenure * daysPerYear
  const totalInvestment = dailyInvestment * totalDays
  const dailyRate = expectedReturn / 100 / daysPerYear
  const futureValue = dailyInvestment * ((Math.pow(1 + dailyRate, totalDays) - 1) / dailyRate) * (1 + dailyRate)
  ...
  const handleSubmitContact = async (e: React.FormEvent) => {
    ...
    await api.submitLead({
      ...
      name, email, phone,
      monthly_investment: Math.round(dailyInvestment * 30),
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 text-center">
        <h2 className="font-sans text-2xl font-bold text-foreground sm:text-3xl">Daily SIP Calculator</h2>
        <p className="mt-2 text-muted-foreground">See how daily investments can grow into a substantial corpus over time</p>
      </div>
          ...
          {stage === "results" && (
            <motion.div key="results" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
              <div className="text-center">
                ...
                <h3 className="font-sans text-2xl font-bold text-card-foreground">Your Daily SIP of {formatCurrency(dailyInvestment)} could grow to</h3>
              </div>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <ul className="mt-2 space-y-1 text-sm text-primary/80">
                      <li>Daily Investment: {formatCurrency(dailyInvestment)}</li>
                      <li>Duration: {tenure} years ({totalDays.toLocaleString("en-IN")} days)</li>
